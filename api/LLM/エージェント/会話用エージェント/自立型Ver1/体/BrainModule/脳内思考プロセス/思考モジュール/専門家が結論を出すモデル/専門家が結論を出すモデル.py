...
class 専門家が結論を出すモデル(Iキャラ依存思考モデル):
    _llmBox: 切り替え可能LLMBox

    def __init__(self) -> None:
        self._llmBox = 切り替え可能LLMファクトリーリポジトリ.singleton().createLLMs(LLMs用途タイプ.専門家が問に対して結論を出す)

    async def 実行(self, v状況履歴: 状況履歴, v記憶部署: I記憶部署, vキャラクター情報:I自分の情報コンテナ, 追伸:IModelDumpAble|str|None = None) -> BaseModel:
        結果 = await self.専門家同士の会話シミュレート2(v状況履歴, v記憶部署, vキャラクター情報, 追伸)
        while 結果.g結論が出たかまだ続けるかの判断 == ConversationProgressStatus.まだ続ける:
            結果 = await self.専門家同士の会話シミュレート続き(結果)
        return 結果

    async def 問いに答える(self, 問い: IModelDumpAble, 私の名前:str,追伸:IModelDumpAble|str|None = None) -> BaseModel:
        結果 = await self.専門家同士の会話シミュレート(問い,私の名前)
        while 結果.g結論が出たかまだ続けるかの判断 == ConversationProgressStatus.まだ続ける:
            結果 = await self.専門家同士の会話シミュレート続き(結果,追伸)
        return 結果
    ...
    async def 専門家同士の会話シミュレート2(self, v状況履歴: 状況履歴, v記憶部署: I記憶部署, vキャラクター情報:I自分の情報コンテナ, 追伸:IModelDumpAble|str|None = None) -> ExpertConversation:
        proxy = 専門家同士の会話クエリプロキシ2(v状況履歴, v記憶部署, vキャラクター情報, 追伸)
        ExtendFunc.ExtendPrint(proxy.dict化クエリ)
        result = await self._llmBox.llmUnit.asyncGenerateResponse(proxy.json文字列でクエリ出力,ExpertConversation)
    ...
    async def 専門家同士の会話シミュレート続き(self, 専門家同士の会話: ExpertConversation, 追伸:IModelDumpAble|str|None = None) -> ExpertConversation:
