...
class モデル連鎖思考(Iキャラ依存思考モデル):
    ...
    async def 実行(self, v状況履歴:状況履歴, v記憶部署:I記憶部署, vキャラクター情報:I自分の情報コンテナ,追伸:IModelDumpAble|str|None = None) -> BaseModel:
        ...
        自問自答結果:InternalMonologue = await 自問自答モデル.実行(v状況履歴, v記憶部署, vキャラクター情報,追伸)
        結論 = await 結論を出すモデル.問いに答える(自問自答結果,vキャラクター情報.id,追伸)
        報酬 = await v感情ベクトル生成モデル.状況から実際に受け取る報酬を生成する([v状況履歴, v記憶部署, 結論], 追伸)
        return 報酬
