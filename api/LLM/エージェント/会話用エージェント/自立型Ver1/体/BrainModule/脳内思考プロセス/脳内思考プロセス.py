...
class 脳内思考プロセス:
    ...
    async def 脳内思考プロセスを進める(self, 最新メッセージ:list[MessageUnitVer1]):
        ...
        # )
        メッセージクエリ:list[IMessageQuery] = self._外部状況統合所.思考可能な形に状況を整理してメッセージクエリに変換(self._プロセス材料溜め置き場,最新メッセージ)
        new思考状態:思考状態 = await self._llmプロセス計画.llmUnit.asyncGenerateResponse(メッセージクエリ, 思考状態)
