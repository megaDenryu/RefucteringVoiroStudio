...
class 感情ベクトル生成モデル(Iキャラ依存思考モデル):
    _llmBox: 切り替え可能LLMBox
    def __init__(self):
        self._llmBox = 切り替え可能LLMファクトリーリポジトリ.singleton().createLLMs(LLMs用途タイプ.感情ベクトル生成)

    async def 実行(self, v状況履歴: 状況履歴, v記憶部署: I記憶部署, vキャラクター情報:I自分の情報コンテナ, 追伸:IModelDumpAble|str|None = None) -> Benefit:
        return await self.生成する(v状況履歴, v記憶部署, vキャラクター情報, 追伸)

    async def 生成する(self, v状況履歴: 状況履歴, v記憶部署: I記憶部署, vキャラクター情報:I自分の情報コンテナ, 追伸:IModelDumpAble|str|None = None) -> Benefit:
        proxy = 報酬ベクトル生成クエリプロキシ.履歴から生成(v状況履歴, v記憶部署, vキャラクター情報, 追伸)
    ...
    async def 状況から実際に受け取る報酬を生成する(self, 状況:list[IModelDumpAble|str], 追伸:IModelDumpAble|str|None = None) -> Benefit:
        proxy = 報酬ベクトル生成クエリプロキシ.状況から報酬を生成(状況, 追伸)
        result = await self._llmBox.llmUnit.asyncGenerateResponse(proxy.json文字列でクエリ出力, Benefit)
        if isinstance(result, Benefit):
            return result
        raise TypeError("思考ノードの結果がlistではありません")
    
    async def 予測された未来から受け取れる報酬を予測する(self, 未来予測:list[IModelDumpAble|str], 追伸:IModelDumpAble|str|None = None) -> Benefit:
        proxy = 報酬ベクトル生成クエリプロキシ.状況から報酬を生成(未来予測, 追伸)
        result = await self._llmBox.llmUnit.asyncGenerateResponse(proxy.json文字列でクエリ出力, Benefit)
        if isinstance(result, Benefit):
            return result
        raise TypeError("思考ノードの結果がlistではありません")
