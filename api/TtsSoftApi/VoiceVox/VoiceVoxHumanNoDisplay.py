...
class VoiceVoxHumanNoDisplay(HasTTSState):
    ...
    def start(self):
        VoiceVoxHumanNoDisplay.createVoiceVoxNameToNumberDict() #キャラアプデ処理旧
    ...
    def updateAllCharaList(self)->bool:
        return VoiceVoxHumanNoDisplay.updateAllCharaListStatic()
    ...
    def updateAllCharaListStatic()->bool:
        ...
        try:
            #1. VoiceVoxのキャラクター名を取得
            speaker_dict = VoiceVoxHumanNoDisplay.getSpeakerDict()
            #2. キャラクター名リストを更新
            VoiceVoxHumanNoDisplay.updateCharaNames(speaker_dict)
            #3. キャラクター名からボイスモード名リストを返す辞書    を更新
            VoiceVoxHumanNoDisplay.updateVoiceModeInfo(speaker_dict)
            #4. キャラクター名から立ち絵のフォルダ名リストを返す辞書を更新
            VoiceVoxHumanNoDisplay.upadteNicknameAndHumanImagesFolder(speaker_dict)
            return True
